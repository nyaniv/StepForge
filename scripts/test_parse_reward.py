...
# Guarded __main__: compute_parse_reward now uses a spawn-context worker pool,
# and spawn re-imports __main__ in each child. Without this guard, every pool
# worker re-executes the entire test script — symptom is the parent's Test 2
# "FAIL" while the worker child first re-runs all 4 tests with an empty pool.
def _main():
